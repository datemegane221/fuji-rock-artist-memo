...
- `data.eventName`（例: "フジロック2026"）は、上記いずれの場合も自動で開く視聴記録フォームの「イベント名」欄にそのまま自動入力される
- `data.stages`（`{ rawText, month, day, dayOfWeek, startTime, endTime, stage, date }`の配列、`date`は`"2026-07-24"`形式）で出演日程を表す
  - 1件だけの場合：その`date`と`stage`を視聴記録フォームの「日付」「ステージ」欄にそのまま自動入力する
  - 複数件ある場合：自動入力はせず、`rawText`をラベルにした候補チップとして表示する。タップした候補の`date`・`stage`がそれぞれ「日付」「ステージ」欄に入る
