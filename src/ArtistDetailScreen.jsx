...
export default function ArtistDetailScreen({
  ...
  const submitProfile = () => {
    ...
    onUpdateArtist({ ...profileForm, name: profileForm.name.trim() });
  ...
  const submitSighting = () => {
    if (!sightingForm.eventName.trim()) return;
    if (editingSightingId) onUpdateSighting(editingSightingId, sightingForm);
    else onAddSighting(sightingForm);
  ...
  const handleDeleteArtist = () => {
    if (window.confirm(`「${artist.name}」を削除しますか？視聴履歴もすべて削除されます。`)) {
      onDeleteArtist();
    }
  };
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
            ...
            <button onClick={handleDeleteArtist}
              style={{ border: "1px solid rgba(245,243,236,0.4)", background: "rgba(245,243,236,0.15)", color: "#F5C6B8", borderRadius: 20, padding: "5px 12px", fontSize: 12, cursor: "pointer" }}>
              削除
            </button>
          </div>
        </div>
          ...
          <div style={{ background: "white", borderRadius: 12, padding: "1.25rem", marginBottom: "1.5rem", border: "1px solid #E3DFD1" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={submitProfile} disabled={!profileForm.name.trim()}
                  style={{
                    flex: 1, padding: "11px", borderRadius: 8, border: "none", fontSize: 14,
                    background: profileForm.name.trim() ? "#2D4A3E" : "#D3CFC1",
                    color: "white", cursor: profileForm.name.trim() ? "pointer" : "not-allowed", fontWeight: 500,
                  }}>
                  保存する
                </button>
        ...
        {showSightingForm && (
          <div style={{ background: "white", borderRadius: 12, padding: "1.25rem", marginBottom: "1.5rem", border: "1px solid #E3DFD1" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={submitSighting} disabled={!sightingForm.eventName.trim()}
                  style={{
                    flex: 1, padding: "11px", borderRadius: 8, border: "none", fontSize: 14,
                    background: sightingForm.eventName.trim() ? "#2D4A3E" : "#D3CFC1",
                    color: "white", cursor: sightingForm.eventName.trim() ? "pointer" : "not-allowed", fontWeight: 500,
                  }}>
                  {editingSightingId ? "更新する" : "登録する"}
                </button>
                <button onClick={() => { setShowSightingForm(false); setEditingSightingId(null); }}
                  style={{ padding: "11px 16px", borderRadius: 8, border: "1px solid #D3CFC1", fontSize: 14, background: "white", color: "#6B6656", cursor: "pointer" }}>
                  キャンセル
                </button>
              </div>
        ...
        {sorted.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {sorted.map((s) => {
              ...
              return (
                <div key={s.id} style={{ border: "1px solid #E3DFD1", borderLeft: "4px solid #6B5744", borderRadius: 10, padding: "1rem 1.1rem", background: "white" }}>
